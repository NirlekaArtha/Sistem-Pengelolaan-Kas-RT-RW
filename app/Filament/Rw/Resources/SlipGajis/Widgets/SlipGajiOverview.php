...
class SlipGajiOverview extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        $totalGajiBulanIni = SlipGaji::whereHas('petugas', function ($query) use ($rw) {
        ...
        $petugasSudahDigaji = SlipGaji::whereHas('petugas', function ($query) use ($rw) {
                $query->where('id_rw', $rw->id);
            })
            ->whereYear('tanggal', now()->year)
            ->whereMonth('tanggal', now()->month)
            ...
            ->count('id_petugas');
        return [
            Stat::make('Jumlah Gaji Bulan Ini', 'Rp ' . number_format($totalGajiBulanIni, 0, ',', '.'))
                ->description('Total pengeluaran gaji petugas bulan ini')
                ->descriptionIcon('heroicon-m-banknotes')
                ->color('success'),
            Stat::make('Petugas Sudah Gaji', "{$petugasSudahDigaji} dari {$totalPetugas} Petugas")
